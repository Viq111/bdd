...
// bdd CLI commands.
//
// schemaDDL is a literal copy of internal/schema/migrations/0001_initial.sql
// from bdd-8urh (the authoritative storage migration), including its
// built-in status/type seed data. bdd-8urh's migrations live on a separate,
// not-yet-merged branch, so this package cannot import internal/schema
// directly; if the migration changes, re-sync this copy so fixtures stay
// openable by the real bdd binary.
package fixture

// schemaDDL creates every table and index defined by schema version 1, plus
// its built-in status_definitions/type_definitions seed rows.
const schemaDDL = `
...
// schemaVersion is the PRAGMA user_version / schema_versions row that
// applying schemaDDL corresponds to, matching bdd-8urh's schema.CurrentVersion()
// for a fresh database at migration 0001.
